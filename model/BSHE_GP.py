...
def get_basis(S, L, kernel, dtype=torch.float32):
    ...
    B, lamb_sqrt = ker_approx(kernel, S, S_c, dtype) #ls, var, nu
    return B, lamb_sqrt
...
class BSHE_voxel_GP():
    ...
    def update_eta(self):
        ...
        sig2 = 1 / ( self.B_lamb_inv_sumv_ssq / self.sig2_e + 1 / self.sig2_eta)
        mu = sig2 * (self.B_lamb_inv_sumv / self.sig2_e  * self.res).sum(1) 
