...
class BSHE_voxel_GP():
    def __init__(self, Y, grids, 
                kernel, kernel_eta=None,
                X=None, L = 10, L_eta=5, 
                burnin=100, thin=1, mcmc_sample=100,
                inclue_intercept=False,
                A=100,dtype=torch.float32,
                ):
    ...
    def run_single_chain(self, chain_id):
        ...
        #print(self.mcmc_alpha)
        return self.get_samples()
    
    def run_multiple_chains(self, n_chains=4, parallel = False):
        if parallel:
    ...
    def update_sig2_eps(self):
        ...
        m = torch.distributions.Gamma(1, 1/self.A + 1 / self.sig2_eps)
    ...
    def update_sig2_beta(self):
        ...
        m = torch.distributions.Gamma(1, 1/self.A + 1 / self.sig2_beta)
    ...
    def update_sig2_alpha(self):
        ...
        m = torch.distributions.Gamma(1, 1/self.A + 1 / self.sig2_alpha)
...
def PPC(data, X,  samples, basis, basis_eta, n_samples=100, dtype=torch.float32):
    """
    ...
    """
    n_chains, n_mcmc, J = samples['alpha'].shape
    N, V = data.shape
    L = basis.shape[1]
    if n_samples > n_mcmc:
    ...
    for s in range(n_chains):
        for i, ind in enumerate(idx):
            ...
            noise = basis @ (torch.randn(L, N) * sig2_eps.sqrt())
